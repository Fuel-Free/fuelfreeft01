...
const AdminleadsForm = () => {
  ...
  const handleSelectCity = async (e) => {
   ...
   const [userPhone,setuserPhone]=useState('')
    const send=(message)=>{
      ...
      window.open(`https://web.whatsapp.com/send?phone=/${userPhone}&text=${encodedMessage}`, '_blank')
    }
    const formateDetails=(data)=>{
      let a = data&&data.map((item)=>{
        return     `\n Hello, here is firm details.\n   ` +
                  `dealer-Name=${item.firmName}\n`+ 
                  `dealer-phoneNo=${item.whatsappNo}\n,` 
    ...
    const getDataFromNumber=async()=>{
      let res=await axios.get(`https://app.fuelfree.in/lead/sendDetailsToUser/${userPhone}`,{
        headers:{
          "Accept":"application/json"
        }
      })
      let result =await res.data
     let data=await result.vendorDetails
     formateDetails(data)
  ...
  return (
    <div id="admin-page-id">
      <ToastContainer />
      <Adminsidebar />
      <div className="admin-dashboard-lead">
        ...
        <button onClick={getDataFromNumber} >hit this to send message to user about dealer</button>
        ...
        >
          <div className="shre-hover-btnss">
            {" "}
            <IoIosShareAlt />
            <span>this is for leads whatsapp</span>
          </div>
          <div >
